...
def download_site(url, output_folder):
    ...
    # Create folders for CSS, JS, and resources (images, videos)
    ...
    for link in soup.find_all('link', rel='stylesheet'):
        ...
        if file_name:
            ...
            with open(os.path.join(css_folder, file_name), 'r', encoding='utf-8') as css_file:
                ...
                download_background_resources(css_content, css_url, resource_folder)
    ...
    for img in soup.find_all('img'):
        ...
        file_name = save_file(img_url, resource_folder)
        if file_name:
            img['src'] = os.path.join('resources', file_name)

    # Download video sources
    for video in soup.find_all('video'):
        for source in video.find_all('source'):
            video_url = urljoin(url, source['src'])
            file_name = save_file(video_url, resource_folder)
            if file_name:
                source['src'] = os.path.join('resources', file_name)
