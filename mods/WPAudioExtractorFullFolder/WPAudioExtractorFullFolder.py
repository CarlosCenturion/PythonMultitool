...
def procesar_carpeta(ruta_carpeta):
    ...
    for archivo in os.listdir(ruta_carpeta):
        if archivo.endswith(".ogg"):
            ...
            ruta_salida = procesar_archivo(ruta_completa)
            archivos_procesados.append(ruta_salida)
    
    if archivos_procesados:
        unificar = input("¿Desea unificar todas las transcripciones en un solo archivo? (s/n): ").lower()
        if unificar == 's':
            unificar_transcripciones(archivos_procesados, ruta_carpeta)

def unificar_transcripciones(archivos, ruta_carpeta):
    ruta_unificada = os.path.join(ruta_carpeta, "transcripcion_unificada.txt")
    with open(ruta_unificada, "w", encoding="utf-8") as archivo_unificado:
        for archivo in archivos:
            nombre_archivo = os.path.basename(archivo)
            archivo_unificado.write(f"--- Transcripción de {nombre_archivo} ---\n\n")
            with open(archivo, "r", encoding="utf-8") as f:
                archivo_unificado.write(f.read())
            archivo_unificado.write("\n\n")
    print(f"Las transcripciones se han unificado en {ruta_unificada}")
