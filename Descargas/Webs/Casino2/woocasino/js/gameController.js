app.controller('gameCtrl', function ($scope, $http, $log) {
    ...
    $scope.sendForm = function(e) {
        ...
        $.ajax({
			...
			if (Array.isArray(value) && value.some(e => typeof e === 'object')) {
	                        $('.modal__error', $form).show().text('Invalid data type response');
	                        // $('.modal__error', $form).show().text(JSON.stringify(value));
			} else {
